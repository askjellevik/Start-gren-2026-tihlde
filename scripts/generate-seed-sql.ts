...
const lines: string[] = [
  ...
  '',
  'insert into public.settings (id, national_average_kg, national_average_source, national_average_source_url, baseline_kg, baseline_label)',
  `values (true, ${sql(settings.nationalAverageKg)}, ${sql(settings.nationalAverageSource)}, ${sql(settings.nationalAverageSourceUrl)}, ${sql(settings.baselineKg)}, ${sql(settings.baselineLabel)})`,
